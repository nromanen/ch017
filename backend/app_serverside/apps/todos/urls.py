...
from apps.todos.views import UserHandler, TodoHandler, OneUserHandler


user_handler = Resource(UserHandler)
todo_handler = Resource(TodoHandler)
one_user_handler = Resource(OneUserHandler)

urlpatterns = patterns('',
   url(r'^user/(?P<login>[^/]+)/(?P<password>[^/]+)/', one_user_handler),
